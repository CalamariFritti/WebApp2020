...
var util_old = {
  ...
  */
  isNonNegativeInteger: function (x) {
    return Number.isInteger(Number.parseInt(x)) && x >= 0;
  },
 /**
  ...
  */
   ...
   *
   * @param {object} listEl  A list element
   * @param {object} selection  An object map used to fill the association list 
   * @param {string} keyProp  The standard identifier property of the range object type
    ...
    for (p in obj) {
      if (obj.hasOwnProperty(p)) {
        ...
        if (typeof val === "number" ||
            typeof val === "string" ||
            typeof val === "boolean" ||
            util.typeName(val) === "Function" ||
            util.typeName(val) === "Date") {
          clone[p] = val;
        }
